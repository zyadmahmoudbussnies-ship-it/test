...
(async()=>{
  ...
  initUI(profile);
  await Promise.all([loadVideos(profile),loadMaterials(profile)]);
...
async function loadMaterials(profile){
  ...
  const {data:assignments,error}=await supabaseClient
    ...
    .select(`material_id, materials(id,title,description,file_url,file_type,created_at)`)
    .eq("student_id",profile.id);
  if(error) return;
  const materials=(assignments||[]).map(a=>a.materials).filter(Boolean)
    .sort((a,b)=>new Date(b.created_at)-new Date(a.created_at));
  if(materials.length===0){ materialsSection.style.display="none"; return; }
  materialsSection.style.display="block";
  materialGrid.innerHTML=materials.map(buildMaterialCard).join("");
}

function buildMaterialCard(m){
  const icon=m.file_type==="pdf"?"📕":"📘";
  const label=m.file_type==="pdf"?"PDF":"Word";
  const date=new Date(m.created_at).toLocaleDateString("ar-EG",{year:"numeric",month:"long",day:"numeric"});
  return `<div class="material-card">
    ...
    <div class="m-body">
      ...
      <div class="meta-row">
        ...
        <a class="btn btn-primary btn-sm" href="${escAttr(m.file_url)}" target="_blank" rel="noopener">⬇ تحميل</a>
      </div>
    </div>
  </div>`;
}
